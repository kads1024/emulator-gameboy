...
This decision would be reversed only if the project changed its goals and explicitly chose to trade M-cycle accuracy for performance because the chosen timing model could not meet the performance floor defined in `docs/scope.md`. No such trade is currently sanctioned.
